...
void csk_makekey(char *passphrase, unsigned char **key) {
  ...
  *key = ucmalloc(crypto_secretbox_KEYBYTES);
...
size_t csk_sodium_mac(unsigned char **cipher,
  ...
  csk_pad_prepend(&pad_clear, cleartext, crypto_secretbox_ZEROBYTES, clearsize);

  crypto_secretbox(pad_cipher, pad_clear, clearsize + crypto_secretbox_ZEROBYTES, nonce, key);

  csk_pad_remove(cipher, pad_cipher, crypto_secretbox_BOXZEROBYTES, (clearsize + crypto_secretbox_ZEROBYTES) - crypto_secretbox_BOXZEROBYTES);
...
int csk_sodium_verify_mac(unsigned char **cleartext, unsigned char* message,
		       size_t messagesize, unsigned char *nonce, unsigned char *key) {
  // verify the mac
  unsigned char *pad_cipher;
  unsigned char *pad_clear;
  int success = -1;

  csk_pad_prepend(&pad_cipher, message, crypto_secretbox_BOXZEROBYTES, messagesize);
  ...
  csk_pad_remove(cleartext, pad_clear, crypto_secretbox_ZEROBYTES, messagesize);
